...
void agregarZonaTematica(struct Parque* parque) {
    ...
    zona = (struct ZonaTematica*)
        malloc(sizeof(struct ZonaTematica));

    nuevoNodo = (struct NodoZona*)
        malloc(sizeof(struct NodoZona));
    ...
    scanf("%s", zona->tematica);

    printf("Ingrese horario: ");
    scanf("%s", zona->horario);
...
void agregarVisitanteFila(struct Parque* parque) {
    ...
    if (atraccion->fila.frente == NULL)
        atraccion->fila.frente = visitante;

    if (atraccion->fila.final != NULL)
        atraccion->fila.final->sig = visitante;
...
struct NodoEntrada* insertarEntradaABB(
    struct NodoEntrada* raiz,
    struct NodoEntrada* nuevaEntrada
) {
...
void agregarEntrada(struct Parque* parque) {
    ...
    entrada = (struct NodoEntrada*)
        malloc(sizeof(struct NodoEntrada));
    ...
    printf(
        "Entrada agregada correctamente al ABB.\n"
    );
}
...
void menuAgregar(struct Parque* parque) {
    ...
    do {
        ...
        if (opcion == 1)
            agregarZonaTematica(parque);

        if (opcion == 2)
            agregarAtraccion(parque);

        if (opcion == 3)
            agregarVisitanteFila(parque);

        if (opcion == 4)
            agregarEntrada(parque);
...
int main() {
    ...
    parque = (struct Parque*)
        malloc(sizeof(struct Parque));
